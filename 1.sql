CREATE TABLE SPORT(
  ...
  /* Add some basic parameters - min/max players - valid player configurations - nullable - non neg */
  /* Add a max waitlist number for each sport */
...
CREATE TABLE PROFILE(
  ...
  /* Store the payment_receipt base 64 encoded string */
  FOREIGN KEY (currant_user_id) REFERENCES currant_user (currant_user_id)
  /**
    Add a list of push notification ids varchar (max length of a GUID)
   */
);
...
CREATE TABLE CLUB (
  ...
  /*
  This should be a pointer to a table for icons (icon_table)
  - should have icon URL, locked (bool), id
  - need to store back_color and front_color on the club
  */
  image_url VARCHAR(50), /* TODO: Use the URL Data type */
